...
define(['hessian/util', 'utf8'], function(HessianUtil, utf8) {
	...
	switch (true) {
		...
		for (var i = 0; i < stringBuffer.length; i++) {
		    var currentValue = stringBuffer[i];
		    if (currentValue <= 127) { //Parsing unicode values smaller than 127. One byte
			deserializedString += utf8.parse([currentValue]);
		    }
		    else if (currentValue > 127) { //Parsing unicode values larger than 127, Two bytes			
			stringLength++; //The string length, oddly, contains the char length, no the byte length. So, for two bytes chars, we need to increase the reported length
			stringBuffer = new Uint32Array(workingBuffer.subarray(currentOffset + 1, currentOffset + stringLength + 1));//Update the string Buffer
			var charArray = stringBuffer.subarray(i, i + 2);
			deserializedString += utf8.parse(charArray);
			i++; //Increment i, so we dont parse the next byte 
		    }
		}
		...
		for (var i = 0; i < stringBuffer.length; i++) {
			...
			var charArray = stringBuffer.subarray(i, i + 2);
			deserializedString += utf8.parse(charArray);
			i++; //Skip the next cycle in for
			stringLength++; //The string length, oddly, contains the char length, no the byte length. So, for two bytes chars, we need to increase the reported length
			stringBuffer = new Uint32Array(workingBuffer.subarray(currentOffset + 2, currentOffset + stringLength + 2));
		    }
		}
		...
		throw new Error("'reserved' data type is not implemented. value 0x"
			+ workingBuffer[currentOffset].toString(16) + " address 0x" + currentOffset.toString(16));
		break;
	    case (workingBuffer[currentOffset] === 0x46) :
		throw new Error("'boolean False' data type is not implemented. value 0x"
			+ workingBuffer[currentOffset].toString(16) + " address 0x" + currentOffset.toString(16));
		break;
		...
		for (var i = 0; i < numberOfItems.value; i++) {
			...
			throw new Error("fixed-length list/vector ('V') List element " + i + " parsed incorrectly. address 0x"
		 ...
		 */
		//This formula is in the spec for compact int
		var longValue = ((workingBuffer[currentOffset] - 0xf8) << 8) + workingBuffer[currentOffset + 1];

		//Keep decoding the object
		return {length: 2,
		    value: longValue,
		    type: 'number',
		    encodedAs: "two-octet compact long (-x800 to x7ff, xf8 is 0)"
		};
		break;
	}
    }
